...
def visao_geral_vendedores(request):
    ...
    # Mapear canal EXTERNO -> PAP (mesma regra da tua view)
    ...
    # Filtro por período para cada métrica
    ...
    # Listas para selects (igual cidades)
    ...
    # Aplicar filtros selecionados (nas bases do período)
    if regionais:
    ...
    # === Agregar por VENDEDOR (consolidado) ===
    # Realizado: adesão = contagem de linhas (cada linha = venda)
    ad_real_vend = (
    ...
    # Realizado: ativação = contagem de linhas com data válida
    at_real_vend = (
    ...
    # Base de vendedores (inclui quem tem só adesão ou só ativação)
    vendedores = sorted(
    ...
    # === Projeções via DiasUteis (igual tua lógica) ===
    ...
    context = {
        'tabela': tbl.rename(columns={
            'vendedores':'vendedor'
        }).to_dict(orient='records'),

        # Totais (para cards no topo)
        ...
        # Filtros/periodo (para manter o mesmo form da outra tela)
